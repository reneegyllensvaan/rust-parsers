...
pub type ParseFn<'a> = fn(&'a str) -> ParseResult<'a>;

// pub struct Parser<'a> {
//     parser: RefCell<Box<dyn Fn(&'a str) -> ParseResult<'a> + 'a>>,
// }
...
//         (self.parser.borrow())(head)
//     }
// }

// pub fn parse_literal<'a, Prod>(lit: &'static str, production: &'a Prod) -> Parser<'a>
// where
//     Prod: Fn() -> SexpNode + 'a,
// {
//     Parser::new(move |head| {
//         Ok(match head.get(0..lit.len()) {
//             Some(s) if s == lit => Some((production(), &head[lit.len()..])),
//             _ => None,
//         })
//     })
// }
// pub fn coalesce<'a>(prefer: &'a Parser<'a>, otherwise: &'a Parser<'a>) -> Parser<'a> {
//     Parser::new(move |head| match prefer.parse(head)? {
